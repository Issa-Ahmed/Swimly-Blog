...
<section class="ml-72 py-24 pl-10">
    ...
    <form action="{{route('dashboard.tags.update', $tag->id)}}" method="POST">
        ...
        <div class="py-4">
          ...
          <input type="text" class="w-64 h-8 rounded border border-gray-500 pl-3 text-lg" name="name"  value="{{ $tag->name }}" required>
        </div> 
        <button type="submit" class="px-12 rounded py-2 bg-green-500 font-bold text-white">Update Tag</button>
    </form>
</section>
@endsection